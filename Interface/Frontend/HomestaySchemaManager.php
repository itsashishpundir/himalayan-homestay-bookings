...
class HomestaySchemaManager {
	...
	private static function output_single_homestay_tags(): void {
		...
		$base_price    = (float) get_post_meta( $post_id, 'base_price_per_night', true );
		$offer_price   = (float) get_post_meta( $post_id, 'offer_price_per_night', true );
		$display_price = ( $offer_price && $offer_price < $base_price ) ? $offer_price : $base_price;
		$currency      = get_post_meta( $post_id, 'currency', true ) ?: 'INR';
		$max_guests    = (int) ( get_post_meta( $post_id, 'max_guests', true ) ?: 6 );
		...
		// --- Meta description ---
		$meta_desc = sprintf(
			'Book %s in %s. From %s %s/night. Sleeps %d guests.%s Authentic Himalayan homestay experience.',
			$property_name,
		...
		if ( $display_price > 0 ) {
			$schema['priceRange'] = $currency . ' ' . number_format( $display_price ) . '/night';
		}
